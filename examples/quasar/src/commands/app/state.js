...
export default {
  guide: 'Get or set values in the app state: state [get/set] [path] [value]',
  documentation: `
# State Command

This command allows you to view or modify the application state values using dot notation paths.

## Syntax
\`\`\`
state get [path]       # Get a value at the specified path
state set [path] [value] # Set a value at the specified path
\`\`\`

## Parameters
- \`get/set\`: The operation to perform (required)
- \`path\`: Dot notation path to the property (required)
- \`value\`: New value when using 'set' operation (required for 'set')

## Examples
\`\`\`
state get settings.layout.header         # Get the header settings
state set settings.layout.header.visible true   # Set header visibility to true
state get user.preferences               # Get all user preferences
state set theme.darkMode true            # Enable dark mode
\`\`\`
  `,
  command: function(args) {
    if (!args || args.length < 2) {
      ...
      const responseObject = {
        response: {
          error: true,
          message: "Insufficient arguments. Usage: state [get/set] [path] [value]"
        },
        meta: {
          command: "state",
          args: args || [],
